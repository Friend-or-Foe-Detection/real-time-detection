...
Usage:
  ...
  2. Run the script:
       python train_compare_yolov8s3.py \
         --epochs 5 \
         --batch-size 4 \
         --imgsz 640
"""
...
def main(args):
    print(f"--- Starting Reproduction of compare_yolov8s3 ---")
    ...
    results = model.train(
        ...
        device=device_arg,
        project="runs/detect",
        name="compare_yolov8s3_reproduced"
    )
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Reproduce YOLO training run compare_yolov8s3.")
